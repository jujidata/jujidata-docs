...
2.  选择“带掩码的语义分割”标注模式，从标签列表中选择当前要标注的目标类别；
3.  使用掩码画笔在图像上涂抹目标区域，可调整画笔大小，支持擦除、撤销操作，确保目标区域被完整覆盖；
4.  完成掩码涂抹后，点击确认，即可生成该目标的语义分割标注；
...
<!-- 以下关键字和标签等，可设计为跳转链接，跳转到本网站的相关内容，例如：[BrushLabels]() -->
