---
title: "基因视觉 (Visual Genome)"
description: "基因视觉标注模版，支持区域、属性、关系三类标签分栏标注，适配图像语义理解与场景关系建模任务。"
lead: "基因视觉标注用于构建更丰富的图像语义数据，不仅标注区域目标，还可同步描述目标属性与目标间关系，适用于场景理解、图文推理与多模态模型训练。"
...
params:
  seo:
      title: "基因视觉 (Visual Genome)"
      description: "基因视觉标注模版，支持区域、属性、关系三类标签分栏标注，适配图像语义理解与场景关系建模任务。"
      keywords: ["标注模板", "基因视觉", "Visual Genome"]
      canonical: "https://docs.jujidata.com/docs/templates/visual-genome"
      robots: "index, follow"
---

# 基因视觉使用说明

基因视觉可以理解为“先依次选好区域、属性、关系，再框选图像”：按顺序在「区域」「属性」「关系」三栏中分别点选对应标签后，再在图像上用矩形框标出目标区域；框选完成后，系统会结合所选标签自动生成与该区域相关的描述信息。它适合复杂场景理解任务，比如图像描述生成、视觉问答、关系推理与多模态训练等场景。
...
![基因视觉标注示例](./images/visual-genome-example.png)
...
![基因视觉模板预览](./images/visual-genome.png)
