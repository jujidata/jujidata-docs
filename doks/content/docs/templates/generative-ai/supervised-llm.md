---
title: "监督式语言模型微调 (Supervised LLM Fine-tuning)"
description: "监督式 LLM 微调标注模版，以卡片样式展示提示（prompt），并在下方文本区收集参考答案，适配指令微调与代码生成等监督数据构建。"
lead: "将「提示」与「期望输出」固定在同一任务中，由标注员撰写或补全答案，可直接用于监督微调、偏好对比前的基线答案等场景。"
...
params:
  seo:
      title: "监督式语言模型微调 (Supervised LLM Fine-tuning)"
      description: "监督式 LLM 微调标注模版，以卡片样式展示提示（prompt），并在下方文本区收集参考答案，适配指令微调与代码生成等监督数据构建。"
      keywords: ["标注模板", "监督微调", "SFT", "LLM", "生成式 AI", "指令跟随"]
      canonical: "https://docs.jujidata.com/docs/templates/generative-ai/supervised-llm"
      robots: "index, follow"
---
