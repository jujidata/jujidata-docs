---
title: "自动语音识别 (Automatic Speech Recognition)"
description: "在可缩放波形上播放音频，将听写结果录入与音频对象绑定的文本框；适合整段转写、ASR 质检与指令数据准备。"
lead: "任务数据通过 audio_path 加载音频；TextArea 绑定 Audio，maxSubmissions 限制提交条数，便于一条任务一份转写。"
...
params:
  seo:
      title: "自动语音识别 (Automatic Speech Recognition)"
      description: "在可缩放波形上播放音频，将听写结果录入与音频对象绑定的文本框；适合整段转写、ASR 质检与指令数据准备。"
      keywords: ["标注模板", "语音识别", "ASR", "Audio", "转写", "音频"]
      canonical: "https://docs.jujidata.com/docs/templates/audio-speech-processing/automatic-speech-recognition"
      robots: "index, follow"
---
