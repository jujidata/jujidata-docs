---
title: "声音事件监测 (Sound Event Detection)"
description: "在波形上为不同「事件」类型划时间区间，用颜色区分事件一/事件二；无转写、无意图选项，专用于强监督的声音事件起止与类别标注。"
lead: "Labels 与 Audio 绑定，在时间轴上拖选区段；事件类型数量可按 Label 增删，适配车声/门铃/关键词段等事件检测数据。"
date: 2026-04-24 08:00:00
lastmod: 2026-04-24 08:00:00
draft: false
weight: 6
menu:
  sidebar_docs:
...
params:
  seo:
      title: "声音事件监测 (Sound Event Detection)"
      description: "在波形上为不同「事件」类型划时间区间，用颜色区分事件一/事件二；无转写、无意图选项，专用于强监督的声音事件起止与类别标注。"
      keywords: ["标注模板", "声音事件", "Sound Event Detection", "Audio", "Labels", "时序检测"]
      canonical: "https://docs.jujidata.com/docs/templates/audio-speech-processing/sound-event-detection"
      robots: "index, follow"
---
