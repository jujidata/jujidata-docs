---
title: "时间序列预测 (Time Series Forecasting)"
description: "在股价等单通道序列上框选「可预测区间」，再在底部选择对下一趋势的上涨/下跌/持平判断；含说明 Header 与底部概览导航。"
lead: "顶部提示框选可预测区间，中部展示时间序列与区域标注，底部单独给出趋势选项，适合将「区间标注 + 任务级趋势标签」一并导出的预测类数据集。"
...
params:
  seo:
      title: "时间序列预测 (Time Series Forecasting)"
      description: "在股价等单通道序列上框选「可预测区间」，再在底部选择对下一趋势的上涨/下跌/持平判断；含说明 Header 与底部概览导航。"
      keywords: ["标注模板", "时间序列预测", "TimeSeries", "股价", "趋势", "预测"]
      canonical: "https://docs.jujidata.com/docs/templates/time-series-analysis/time-series-forecasting"
      robots: "index, follow"
---
