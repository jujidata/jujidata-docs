---
title: "活动识别 (Activity Recognition)"
description: "在多通道时间序列上沿时间轴做区域标注，为每段赋予跑、走、飞等活动类别，支持概览条导航与通道同步缩放。"
lead: "从 URL 加载 CSV 时间序列，展示速度与加速度等曲线；标注员选择标签后在时间轴上框选区间，导出为带起止时刻的活动分段数据。"
...
params:
  seo:
      title: "活动识别 (Activity Recognition)"
      description: "在多通道时间序列上沿时间轴做区域标注，为每段赋予跑、走、飞等活动类别，支持概览条导航与通道同步缩放。"
      keywords: ["标注模板", "活动识别", "TimeSeries", "时间序列", "区域标注", "传感器"]
      canonical: "https://docs.jujidata.com/docs/templates/time-series-analysis/activity-recognition"
      robots: "index, follow"
---
