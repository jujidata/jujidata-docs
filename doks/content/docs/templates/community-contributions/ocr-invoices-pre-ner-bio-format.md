---
title: "OCR 发票预命名实体（BIO 格式） (OCR Invoices — Pre-NER BIO)"
description: "在票据图像上框选「分词单元」矩形，并对每个框填写或校正 OCR 文本；适合先定词界再进入纯文本 NER / BIO 流水线的两阶段方案。"
lead: "Image 展示小票或发票扫描件；RectangleLabels 标出分词级区域，TextArea 按区域录入转写，与后续纯文本 BIO 标注或模型训练衔接。"
...
params:
  seo:
      title: "OCR 发票预命名实体（BIO 格式） (OCR Invoices — Pre-NER BIO)"
      description: "在票据图像上框选「分词单元」矩形，并对每个框填写或校正 OCR 文本；适合先定词界再进入纯文本 NER / BIO 流水线的两阶段方案。"
      keywords: ["标注模板", "社区贡献", "OCR", "发票", "RectangleLabels", "BIO", "NER"]
      canonical: "https://docs.jujidata.com/docs/templates/community-contributions/ocr-invoices-pre-ner-bio-format"
      robots: "index, follow"
---
